...
/*1*/
Select AUDITORIUM_TYPE.AUDITORIUM_TYPENAME, AUDITORIUM.AUDITORIUM, AUDITORIUM.AUDITORIUM_CAPACITY, AUDITORIUM.AUDITORIUM_NAME, AUDITORIUM.AUDITORIUM_TYPE
...
/*3*/
SELECT FACULTY.FACULTY AS Факультет, PULPIT.PULPIT AS Кафедра, GROUPS.PROFESSION AS Специальность, SUBJECT.SUBJECT AS Дисциплина, STUDENT.NAME AS Студент,
      CASE 
...
/*4*/
Select PULPIT.PULPIT_NAME, isnull (TEACHER.TEACHER_NAME,'***')[TEACHER.NAME]
...
CREATE TABLE TIMETABLE (	
	  ...
	  SUBJECT   char(10) constraint TIMETABLE_SUBJECT_FK foreign key
                      references SUBJECT(SUBJECT),
      TEACHER_NAME   char(10) constraint TIMETABLE_TEACHER_FK foreign key         
                      references TEACHER(TEACHER),
      DAY_OF_WEEK nvarchar(15), 
	  CLASS int,
); 

	insert into TIMETABLE   ( IDGROUP, AUDITORIUM, SUBJECT,  TEACHER_NAME, DAY_OF_WEEK, CLASS)
	  values  (2, '236-1','БД','ЖЛК','Понедельник', 1),
			  (2, '301-1','ПСП','ГРН','Понедельник', 2),
			  (3, '301-1','ПСП','ГРН','Понедельник', 3),
			  (4, '423-1','ОАиП','РЖК','Вторник', 1),
			  (7, '236-1','БД','ЖЛК','Вторник', 1),
			  (8, '206-1','ПСП','УРБ','Вторник', 1),
			  (4, '236-1','ПЗ','РВКЧ','Вторник', 2),
			  (5, '413-1','ЭП','МРЗ','Вторник', 3),
			  (6, '301-1','ПСП','ГРН','Среда', 3),
			  (6, '324-1','ТОПИ','УРБ','Четверг', 2)

	delete from TIMETABLE

	SELECT  AUDITORIUM.AUDITORIUM FROM AUDITORIUM WHERE AUDITORIUM.AUDITORIUM NOT IN (
		SELECT TIMETABLE.AUDITORIUM
		FROM TIMETABLE
		WHERE TIMETABLE.DAY_OF_WEEK = 'Вторник'
			AND TIMETABLE.CLASS = 1
	);


	SELECT TEACHER.TEACHER FROM TEACHER WHERE TEACHER.TEACHER NOT IN (
		SELECT  TIMETABLE.TEACHER_NAME
		FROM TIMETABLE 
		WHERE TIMETABLE.DAY_OF_WEEK = 'Понедельник'
			AND TIMETABLE.CLASS = 3
	);

	SELECT GROUPS.IDGROUP FROM GROUPS WHERE GROUPS.IDGROUP NOT IN (
		SELECT  TIMETABLE.IDGROUP
		FROM TIMETABLE
		WHERE TIMETABLE.DAY_OF_WEEK = 'Понедельник'
			AND TIMETABLE.CLASS = 2
	);