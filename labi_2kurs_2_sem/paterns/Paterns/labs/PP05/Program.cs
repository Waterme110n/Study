...
namespace PP05
{
	internal class Program
	{
		static void Main(string[] args)
		{
			...
			{
				IWriter writer = new FileWriter(@"C:\my\study\design_patterns\labs\PP05\test2.txt");
				IWriter rsa = new DecRSA(writer);
				IWriter hash = new DecSHA512(rsa);
			...
			{
				IWriter writer = new FileWriter("C:\\my\\study\\design_patterns\\labs\\PP05\\test4.txt");
				IWriter rsa = new DecRSA(writer);
				IWriter hash = new DecMD5(rsa);
			...
			{
				IWriter writer = new FileWriter("C:\\my\\study\\design_patterns\\labs\\PP05\\test12.txt");
				IWriter rsa = new DecRSA(writer);
				IWriter hash = new DecSHA512(rsa);
				string? result = hash.Save("BBBBCCCCC");
			...
			{
				IWriter writer = new FileWriter("C:\\my\\study\\design_patterns\\labs\\PP05\\test13.txt");
				IWriter rsa = new DecRSA(writer);
				IWriter hash = new DecMD5(rsa);
				string? result = hash.Save("BBBBCCCCC");
		...
		//	bool rc = true;
		//	s = s ?? string.Empty;
		//	byte[] sb = Encoding.Default.GetBytes(s);  // Замена Encoding.ASCII на Encoding.Default
		//	SHA512 sha512 = SHA512.Create();
		//	byte[] sbh = sha512.ComputeHash(sb);
		//	rc = sbh.SequenceEqual(hash);
		//	return rc;
