...
BEGIN
    ...
    -- ѕолучаем ID покупател€ по его имени
    SELECT @customerID = CUST_NUM
    FROM CUSTOMERS
    WHERE COMPANY = @customerName;
    
    -- ѕровер€ем, существует ли покупатель
    IF @customerID IS NULL
    BEGIN
        RAISERROR ('ѕокупатель не найден.', 16, 1);
        RETURN;
    END
    
    -- ¬ыводим список заказов и их итоговую стоимость дл€ указанного покупател€
    SELECT O.ORDER_NUM, O.ORDER_DATE, O.AMOUNT
    FROM ORDERS O
    WHERE O.CUST = @customerID;
    
    -- ¬ыводим итоговую стоимость заказов
...
exec GetCustomerOrders 'Midwest Systems';

/*2*/

select * from ORDERS

create function howMuchOrders(
	@custnum int,
	@Startdate date,
	@Lastdate date
)
...
begin
    ...
    BEGIN
		return 0
    END
	select @end = COUNT(*) from ORDERS
		join CUSTOMERS on CUST_NUM = CUST
		where CUST_NUM = @custnum and ORDER_DATE > @Startdate and ORDER_DATE < @Lastdate
	return @end
end;

SELECT dbo.howMuchOrders(2118, '2008-01-01', '2009-01-01') AS OrderCount; 

	select orders.ORDER_DATE from ORDERS
	join CUSTOMERS on CUST_NUM = CUST
	where CUST_NUM = 2118 and ORDER_DATE > '2008-01-01'

drop function howMuchOrders

/*3*/

select * from CUSTOMERS
create procedure AllTovars(
	@NamePorup varchar(20)
)
as
begin
	if not exists (select 1 from Customers where company = @NamePorup)
	begin 
	raiserror('asdas',1,1);
	end;

	select PRODUCT,AMOUNT from PRODUCTS
	join ORDERS on PRODUCT = PRODUCT_ID 
	join CUSTOMERS on CUSTOMERS.CUST_NUM = orders.CUST 
	where COMPANY = @NamePorup
	order by AMOUNT desc;
end;

exec AllTovars 'Midwest Systes';

drop procedure AllTovars

/*4 ???*/
CREATE procedure createCust (
	@Company VARCHAR(20),
	@CustRep INT,
...
BEGIN
	...
	IF NOT EXISTS (SELECT 1 FROM Customers WHERE COMPANY = @Company)
	BEGIN
		INSERT INTO Customers (Company, CUST_REP, CREDIT_LIMIT)
		VALUES (@Company, @CustRep, @CreditLimit);

		SET @Result = (SELECT CUST_NUM FROM Customers WHERE COMPANY = @Company);
	END;

	RETURN @Result;
END;

exec createCust 'sadasd',12,9231
/*5*/
create procedure orders_in_time(
	@startdate date,
	@enddate date
)
as
begin
	...
	select * from CUSTOMERS join ORDERS on CUST_NUM = CUST
	where ORDER_DATE < @enddate and ORDER_DATE > @startdate
	order by AMOUNT desc;
end;

exec orders_in_time '2008-01-01','2009-01-01';

	select * from ORDERS

	drop procedure orders_in_time

/*6*/
create function asdbasdk(
	@startdate date,
	@enddate date
)
returns int
...
begin
	declare @kolvo int;
	select @kolvo =  COUNT(*) from ORDERS where @startdate < ORDER_DATE and @enddate > ORDER_DATE
	return @kolvo
end;
select dbo.asdbasdk('2007-01-01','2008-01-01') ;

select ORDER_DATE from ORDERS

drop function asdbasdk

/*7*/
create procedure asdadlas(
	@startdate date,
	@enddate date)
as 
begin
	select * from CUSTOMERS join ORDERS on CUST = CUST_NUM 
	where @startdate < ORDER_DATE and  @enddate > ORDER_DATE
end;

exec asdadlas '2007-01-01','2008-01-01';

/*8*/
create function asdkadm(
...
begin
	...
	select @result = COUNT(CUST_num) from CUSTOMERS 
	join ORDERS on CUST=CUST_NUM
	join PRODUCTS on PRODUCT = PRODUCT_ID
	where PRODUCTS.DESCRIPTION = @name_tovar

	return @result
end;

select dbo.asdkadm('Size 3 Widget')
select * from PRODUCTS
select * from ORDERS

/*9*/
create procedure ins(
	@tovarsname varchar(20)
)
as
begin
	declare @price money;
	...
	set @price = (select price from PRODUCTS where DESCRIPTION = @tovarsname)
	set @totalmoney = @price/100*110
	update products set price = @totalmoney where DESCRIPTION = @tovarsname;
...
exec ins 'Size 2 Widget'
select * from PRODUCTS

/*10*/
create function qwe123(
	@buyer int,
	@yeartovara date
)
returns int
...
begin
	declare @result int
	select @result = COUNT(ORDER_NUM) from ORDERS
	where YEAR(ORDER_DATE) = Year(@yeartovara) and CUST = @buyer;
	return @result
end;

drop function qwe123

select dbo.qwe123(2103,'2007');
...
as
begin
	declare @seq int = next value for  Myseq
	begin try
	INSERT INTO Customers (CUST_NUM, COMPANY, CUST_REP, CREDIT_LIMIT)
		VALUES (@seq, @Company, @CustRep, @CreditLimit);
