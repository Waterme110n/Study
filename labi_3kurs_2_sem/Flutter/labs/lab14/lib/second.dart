...
class _FoodDetailScreenState extends State<FoodDetailScreen> with SingleTickerProviderStateMixin {
  ...
  late Animation<double> _secrotationAnimation;
  late Animation<Offset> _secslideAnimation;
  late Animation<double> _secscaleAnimation;
  late Animation<double> _secopacityAnimation;
  ...
  void initState() {
    ...
    _opacityAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(parent: _controller, curve: Interval(0, 0.25, curve: Curves.ease)),
    );

    _slideAnimation = Tween<Offset>(begin: Offset(0.0, 0.0), end: Offset(1.0, 0.0)).animate(
      CurvedAnimation(parent: _controller, curve: Interval(0.25, 0.5, curve: Curves.ease)),
    );

    _rotationAnimation = Tween<double>(begin: 0, end: 1).animate(
      CurvedAnimation(parent: _controller, curve: Interval(0.5, 0.75, curve: Curves.ease),),
    );

    _scaleAnimation = Tween<double>(begin: 1.0, end: 2.0).animate(
      CurvedAnimation(parent: _controller, curve: Interval(0.75, 1, curve: Curves.ease)),
    );

    _secrotationAnimation = Tween<double>(begin: 0, end: -1).animate(
      CurvedAnimation(parent: _controller, curve: Interval(0, 0.25, curve: Curves.ease)),
    );

    _secslideAnimation = Tween<Offset>(begin: Offset(3.5, 5.0), end: Offset(3, 2)).animate(
      CurvedAnimation(parent: _controller, curve: Interval(0.25, 0.5, curve: Curves.ease)),
    );

    _secscaleAnimation = Tween<double>(begin: 1.0, end: 0.5).animate(
      CurvedAnimation(parent: _controller, curve: Interval(0.5, 0.75, curve: Curves.ease)),
    );

    _secopacityAnimation = Tween<double>(begin: 1.0, end: 0.0).animate(
      CurvedAnimation(parent: _controller, curve: Interval(0.75, 1.0, curve: Curves.ease)),
    );
        ...
        children: [
          Stack(
            children: [
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 25),
                        Text('Pancakes', style: TextStyle(color: Colors.black,  fontSize: 40, fontFamily: 'Roboto',fontWeight:  FontWeight.bold)),
                        Text(('pancake • banana').toUpperCase(), style: TextStyle(color: Color(0xFFcccccc),  fontSize: 15, fontFamily: 'Roboto',fontWeight:  FontWeight.bold)),
                        SizedBox(height: 20),
                        Container(
                          width: 1000,
                          height: 1,
              ...
              SlideTransition(
                position: _secslideAnimation,
                child: FadeTransition(
                  opacity: _secopacityAnimation,
                  child: RotationTransition(
                    turns: _secrotationAnimation,
                    child: ScaleTransition(
                      scale: _secscaleAnimation,
                      child: Positioned(
                          bottom: 210,
                          right: 50,
                          child: Image.asset(
