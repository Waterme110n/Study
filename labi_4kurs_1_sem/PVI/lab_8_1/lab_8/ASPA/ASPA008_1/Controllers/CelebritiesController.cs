...
namespace ASPA008_1.Controllers
{
    public class CelebritiesController : Controller
    {
        ...
        public IActionResult Human(int id)
        {
            ...
            if (references != null)
            {
                Console.WriteLine($"Контроллер получил {references.Count} ссылок Wikipedia для знаменитости ID={id}");
            }
            else
            {
                Console.WriteLine($"Контроллер: ссылки Wikipedia не найдены для знаменитости ID={id}");
            }
            ...
            FormMode mode)
        {
            ...
            if (press == "Confirm")
            {
                ...
                else // Edit
                {
                    ...
                    repo.UpdateCelebrity(celeb.Id,celeb);
                }
                return RedirectToAction("Index");
            }
